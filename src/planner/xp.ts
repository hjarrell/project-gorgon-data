...
/**
 * Raw XP table entry shape from xptables.json.
 */
interface RawXpTableEntry {
  InternalName: string;
  XpAmounts: number[];
}
 ...
 */
export function buildXpTableLookup(
  raw: Record<string, RawXpTableEntry>,
): Map<string, number[]> {
 ...
 */
export function calcDropOffMultiplier(
  skillLevel: number,
  dropOffLevel?: number | null,
  dropOffRate?: number,
  dropOffPct?: number,
): number {
 ...
 */
export function calcRecipeXp(
  recipe: Recipe,
  skillLevel: number,
  completionCount: number,
  craftingXpMod: number = 1.0,
): number {
  ...
  if (isFirstCraft) {
    ...
    const multiplier = calcDropOffMultiplier(
      skillLevel,
