 ...
 */
export function initSimulation(characterState, targetSkill, options, allRecipes, allSkills, xpTableLookup) {
    // Resolve skill info
    ...
    if (!skillEntry) {
        throw new Error(`Character has no data for skill "${targetSkill}". ` +
    ...
    for (const [recipeId, recipe] of allRecipes) {
        if (recipe.RewardSkill !== targetSkill)
            continue;
        if (!completions.has(recipe.InternalName))
            continue;
        if (excludeRecipes?.has(recipe.InternalName))
            continue;
 ...
 */
export function canCraftRecipe(recipe, state) {
    ...
    if (recipe.MaxUses != null && count >= recipe.MaxUses)
        return false;
 ...
 */
export function scoreCandidate(candidate, state, scorer) {
    if (!canCraftRecipe(candidate.recipe, state))
        return null;
 ...
 */
export function findBestCandidate(state, scorer) {
    let best = null;
    let bestScore = 0;
 ...
 */
export function applyCraft(state, candidate, xp) {
 ...
 */
export function stepSimulation(state, scorer) {
 ...
 */
export function buildPlanResult(state) {
    return {
